...
class Solution {
    ...
    ArrayList<ArrayList<String>> myfindLadders(String start,
        String end, HashSet<String> dict) {
        int i, j, k, lenW = start.length(), diff=0, cur, next;
        boolean found = false;
        ArrayList<ArrayList<String>> collection = 
            new ArrayList<ArrayList<String>>();
        ArrayList<ArrayList<String>> path_set =
            new ArrayList<ArrayList<String>>(); 
        LinkedList<String> queue = new LinkedList<String>();
        String word, new_word;
        ArrayList<String> path = new ArrayList<String>(), new_path;
        HashMap<String, ArrayList<String>> map = 
            new HashMap<String, ArrayList<String>>();
        HashSet<String> cur_level = new HashSet<String>();
        HashSet<String> next_level= new HashSet<String>();

        path.add(start);
        path_set.add(path);

        if(start.equals(end)) return path_set;
        for(i = 0; i < lenW; i++) {
            if(start.charAt(i) != end.charAt(i)) diff += 1;
            if(diff == 2) break;
        }
        if(diff == 1) {
            path.add(end);
        ...
        queue.offer(start);
        k = 0; cur = 1; next = 0;
        while(queue.size() != 0 && cur > 0) {
            word = queue.poll();
            dict.remove(word);
            outerloop:
            for(i = 0; i < lenW; i++) {
                for(j = 0; j < 26; j++) {
                    new_word = word.substring(0,i)+(char)('a'+j)+
                               word.substring(i+1);
                    if(new_word.equals(end)) {
                        if(!found) {
                            new_path = new ArrayList<String>();
                            new_path.add(word);
                            map.put(end, new_path);
                        }else {
                            map.get(end).add(word);
                        }
                        found = true;
                        break outerloop;
                    }else if(!found && dict.contains(new_word) &&
                             !cur_level.contains(new_word)){
                        if(!map.containsKey(new_word)) {
                            new_path = new ArrayList<String>();
                            new_path.add(word);
                            map.put(new_word, new_path);
        ...
        if(!map.containsKey(end)) return collection;
        LinkedList<String> back_path = new LinkedList<String>();
        back_path.push(end);
        genPath(collection, map, end, back_path);
        return collection;
    }
    void genPath(ArrayList<ArrayList<String>> collection,
                 HashMap<String, ArrayList<String>> map, String word,
                 LinkedList<String> path) {
        ArrayList<String> prev = map.get(word);
        for(int i = 0; i < prev.size(); i++) {
            LinkedList<String> new_path = new LinkedList<String>(path);
            new_path.push(prev.get(i));
            if(!map.containsKey(prev.get(i))) {
            //if(prev.get(i).equals(start)) {
                ArrayList<String> arr_path = new ArrayList<String>(new_path);
                collection.add(arr_path);
            }else {
                genPath(collection, map, prev.get(i), new_path);
            }
        }
    }

    ArrayList<ArrayList<String>> slowfindLadders(String start,
        String end, HashSet<String> dict) {
        int i, j, k, lenW = start.length(), diff=0, cur, next;
        boolean found = false;
        ArrayList<ArrayList<String>> collection = 
        ...
        while(queue.size() != 0 && cur > 0) {
            ...
            for(i = 0; i < lenW; i++) {
                for(j = 0; j < 26; j++) {
                    ...
                    if(new_word.equals(end)) {
                        found = true;
                        for(ArrayList<String> tpath : map.get(word)) {
                            new_path = new ArrayList<String>(tpath);
                        ...
                        break;
                             ...
                             !cur_level.contains(new_word)){
                        if(!map.containsKey(new_word)) {
                            path_set = new ArrayList<ArrayList<String>>();
                            for(ArrayList<String> tpath : map.get(word)) {
                                new_path = new ArrayList<String>(tpath);
                                new_path.add(new_word);
                                path_set.add(new_path);
                            }
                            map.put(new_word, path_set);
                            next_level.add(new_word);
                            queue.offer(new_word);
                            next += 1;
                        }else {
...
public class FindLadders {
    public static void main(String[] args) {
        ...
        String[] A = {"hot","hit","hit","hit","hit","hit","nanny"};
        String[] B = {"dog","cog","hop","hat","hit","cog","aloud"};
        String[][] C = {{"hot","dog"},
                        {"bit","hip","bip","bap","cap","bag","cag"},
                        {"hot","hip"},
                        {"hot"},{"cog"},{"hot","dot","dog","lot","log"},
        ...
        ArrayList<HashSet<String>> D = new ArrayList<HashSet<String>>();
        for(String[] dict : C) D.add(new HashSet<String>(Arrays.asList(dict)));
        for(int i = 0; i < A.length ; i++) {
            sol.printAL2(sol.findLadders(A[i],B[i],D.get(i)));
        }
/*
